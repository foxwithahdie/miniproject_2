...
test: # Python3 runs and creates a verilog file with the variable file name, defined in the Makefile, so that you don't have to write a literal directly into SystemVerilog
	python3 -c 'with open("$(test_folder)/setup.v", "w+", encoding="utf-8") as setup: setup.write("module setup (output logic [255*8-1:0] file_name);\n     assign file_name = \"$(test_build_folder)/$(test_filename).vcd\";\nendmodule\n")'
	iverilog -g2012 -o $(test_build_folder)/$(test_filename) $(test_folder)/$(test_filename).sv
	vvp $(test_build_folder)/$(test_filename)
	gtkwave --optimize --giga $(test_build_folder)/$(test_filename).vcd
